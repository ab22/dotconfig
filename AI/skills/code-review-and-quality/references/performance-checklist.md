...
Quick reference checklist for web application performance. Consumed by the `code-review-and-quality` skill as its performance axis.
...
The decision material (which layer, which invalidation strategy, what never to cache) is a design decision, not a checklist item. This section covers the read/write patterns and the checklist.
