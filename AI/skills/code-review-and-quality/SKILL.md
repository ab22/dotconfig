...
whenToUse: Reviewing a change before hand-off or before opening the PR — your own diff, another agent's, or a human's.
---

> **Workspace overrides — these win over the vendored text below.**
>
> This is a **callable capability, not a lifecycle phase**. The
> `plan → implement → review → PR` workflow is unchanged: invoking this skill adds no
> gate, no key to `.agents/verify.yaml`, and no Definition of Done item. The
> **user owns the review gate and every commit** — never run `git commit`,
> `git tag`, or `git push` yourself.
>
> This skill **consumes** the repository's evidence (`dod` / `handoff`, the
> command list in `.agents/verify.yaml`) and supplies the *judgement* on top of
> it. It does not re-run those commands and does not restate the Definition of
> Done.
>
...
This table defines *when the capability is invoked*. It is not a lifecycle phase:
the workflow keeps its single review gate, and the user owns it.

| # | Trigger | Required? | What invoking adds |
| --- | --- | --- | --- |
| T1 | Implementation is complete, at the lifecycle's **review gate**, before hand-off | Always | the findings a hand-off summarizes; the primary invocation |
| T2 | The branch is about to become a **PR** | Always | a last pass over the frozen diff, after review feedback is applied |
| T3 | Reviewing a change **you did not write** — another agent's or a human's | Always | the multi-model review pattern and a fresh set of axes |
...
- **Scope is the branch diff against the base branch** (typically `alpha`), not
  the whole repo, and **the tests are read before the implementation** — they
  reveal intent.
- This skill **consumes** the repository's evidence (`dod` / `handoff`,
  `.agents/verify.yaml`) and never restates the Definition of Done.
...
For triaging `npm audit` findings and supply-chain risk (typosquatting, compromised maintainers), follow `references/security-checklist.md` — this section covers the upgrade *workflow*, that one covers the security verdict.
